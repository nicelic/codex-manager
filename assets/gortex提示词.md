...
全流程原生 MCP 绝对闭环约束：已 track 仓库的一切操作（包括代码检索、语法分析、源码精读、架构评估、以及所有文件编辑、代码修改、文本替换、源码覆盖写入、符号重构与删除），必须且仅能通过 Gortex 原生 MCP 句柄闭环执行。
严禁读写割裂：绝对禁止在阅读时使用 Gortex、代码修改时却擅自退回宿主原生工具。严禁对已 track 仓库调用任何宿主原生工具（包括但不限于 replace_file_content、write_to_file、view_file、grep_search、find_by_name 等）。
不可降级与熔断通知：若 Gortex 原生工具无法使用、报错或无法满足需求，严禁擅自退回任何原生工具自行兜底，必须立即停止操作并向用户说明原因。未 track 的项目或内容不得擅自进行 track 图谱化。
...
唯一法定写契约：对已 track 仓库的文件修改、代码替换、符号重写与全量写入，Gortex 的 edit_file、batch_edit、write_file、edit_symbol 是唯一的法定落地途径。所有写操作必须严格遵守当前 schema、effect、fixed_arguments、guard、view、overlay 和 partial failure 结果，严禁通过宿主写工具绕过 Gortex 状态机与图谱同步。
...
9. wakeup（架构全景摘要）：门面 explore(operation="wakeup")；扁平 gortex_wakeup()。生成约 500 Token 架构摘要 Markdown。
...
| 修改文件 | edit_file(path="...", old_string="...", new_string="...", dry_run=true\|false) | edit(operation="file", target={file:"..."}, dry_run=true\|false) | [热] 预览 dry_run=true 禁带凭据；严禁使用宿主 replace_file_content |
...
| 覆盖写入文件 | write_file(path="...", content="...") | edit(operation="write", target={file:"..."}, content="...") | [热] 全量覆盖写入；严禁使用宿主 write_to_file |
...
代码修改唯一性铁律：对已 track 仓库文件的所有增删改查必须经由 edit 与 refactor 体系完成。严禁调用宿主环境的 replace_file_content、write_to_file 等外挂工具修改已 track 仓库，否则将造成 Gortex 内存图谱、AST 依赖与物理磁盘失步。
...
多仓库全图谱最终铁律：已 track 仓库全部统一托管于全局图谱。严禁在未经跨项目穿透检索或静默切域的情况下断定未索引并退回原生读写工具。全生命周期（探索、定位、精读、修改、重构、验证）全程由 Gortex MCP 工具闭环，严禁读写脱节。严禁调用 replace_file_content、write_to_file、view_file、grep_search 等宿主原生工具。只有在 query_project 与带前缀路径均证实未 track，且用户明确要求本地文件检查时，方可报告未 track 状态。
