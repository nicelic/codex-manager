...
- 确认更新后，前端自动按序调用停止顶部代理、RTK、snip、llmtrim 以及 Gortex daemon；更新脚本执行极简可靠流程：一并强杀四大工具的所有进程、强杀 code-Manager 面板的所有同名进程，直接覆盖替换文件，并按照名称重新启动新版 code-Manager.exe，按需恢复顶部代理并清理事务标记。
- 修复托盘初始化脆弱点：若 Windows 任务栏通知区域暂时异常或添加托盘图标失败，将其作为非致命警告记录，绝不阻断或退出核心 Web 服务，保证 7780 管理页面与代理网关在任何环境下始终稳定可用。
